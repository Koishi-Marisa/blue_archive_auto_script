...
def _isolate_gradle_project():
    """Create a local settings.gradle so Gradle does not walk up to /work.

    The repository root still contains the old Chaquopy project's
    settings.gradle.kts. When buildozer runs gradlew inside
    .buildozer/android/platform/build-*/dists/boa without its own settings
    file, Gradle climbs the parent directories and picks up that root file,
    causing "Project directory ... is not part of the build" errors.
    """
    settings_gradle = os.path.join(os.getcwd(), 'settings.gradle')
    settings_gradle_kts = os.path.join(os.getcwd(), 'settings.gradle.kts')
    if os.path.exists(settings_gradle) or os.path.exists(settings_gradle_kts):
        return
    info(f'Creating isolated settings.gradle in {os.getcwd()}')
    with open(settings_gradle, 'w', encoding='utf-8') as f:
        f.write('// Auto-generated by p4a_hook to isolate this Gradle project.\n')
