...
android {
    namespace = "top.qwq123.baas"
    compileSdk = 37


    defaultConfig {
        applicationId = "top.qwq123.baas"
        minSdk = 28
        targetSdk = 36
        ...
        // setup_baas_assets.py deploy 时写入 .baasversion；缺失时为空串，运行时检查宽松放行
        val baasVersion = rootProject.file(".baasversion")
    ...
    buildTypes {
        release {
            ...
            if (hasReleaseKeystore) {
                ...
                println("[Signing] No release keystore configured, release build will use debug signing")
            }
        }
    }
