"""Interactive normal-stage route test.

Run from the repository root with an emulator already running:

    uv run --no-sync python -m unittest develop_tools.test.test_explore_normal_task

The stages come from ``explore_normal_task_list`` in the selected config and
are forced to run even when they have already been completed.
"""

import unittest

from core.config.config_set import ConfigSet
from baas_main import Main
from module.explore_tasks.explore_task import explore_normal_task

TEST_CONFIG_DIR = "cn"


class TestExploreNormalTask(unittest.TestCase):
    def setUp(self):
        self.main = Main(ocr_needed=["en-us"])
        self.baas_thread = self.main.get_thread(
            ConfigSet(config_dir=TEST_CONFIG_DIR),
            name="explore-normal-test",
        )
        self.assertTrue(
            self.baas_thread.init_all_data(),
            f"Failed to initialize BAAS test config: {TEST_CONFIG_DIR}",
        )

    def tearDown(self):
        self.baas_thread.flag_run = False
        if self.main.ocr is not None:
            self.main.ocr.client.stop_server()

    def test_explore_normal_task(self):
        self.assertTrue(
            explore_normal_task(
                self.baas_thread,
                force=True,
            )
        )


if __name__ == "__main__":
    unittest.main()
